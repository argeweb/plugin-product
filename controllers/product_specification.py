...
def process_spec(spec_name, spec_data):
    """ 格式化產品規格資料、去除空白
...
class ProductSpecification(Controller):
    ...
    # @route_menu(list_name=u'backend', group=u'產品管理', need_hr=True, text=u'產品規格', sort=1206)
    def admin_list(self):
    ...
    def admin_stock_process(self):
        ...
        if order is not None:
            self.fire(
                ...
                uri='taskqueue:product_stock:stock:reset_order_quantity',
    ...
    def admin_side_panel_for_product(self, target=''):
        ...
        total, spec_lists = get_spec_lists([
            process_spec(product_record.spec_1_name, product_record.spec_1),
            process_spec(product_record.spec_2_name, product_record.spec_2),
            process_spec(product_record.spec_3_name, product_record.spec_3),
            process_spec(product_record.spec_4_name, product_record.spec_4),
            process_spec(product_record.spec_5_name, product_record.spec_5)]
        )
        ...
        def query_factory(controller):
            return self.meta.Model.all_with_product(product_record)
    ...
    @route
    def admin_get_spec_detail(self):
        def query_factory(controller):
    ...
    def admin_get_warehouse_detail_with_order(self):
        ...
        for item in order_items:
            sku = item.sku.get()
            data.append(SKUIW_Model.get_or_create(spec=sku, warehouse=warehouse))
